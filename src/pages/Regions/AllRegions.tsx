...
const AllRegions = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${baseUrl}api/regions/get-all-regions/?search=${encodeURIComponent(
          search,
        )}&page=${page}&election_year=${electionYear}`,
        {
          headers: {
  ...
  }, [baseUrl, search, page, electionYear, userToken]);
  ...
  const handleDelete = async (itemId) => {
    ...
    try {
      const response = await fetch(`${baseUrl}api/regions/delete-region/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Token ${userToken}`,
...
>
  ...
  {['1992', '1996', '2000', '2000R', '2004', '2008', '2008R', '2012', '2016', '2020', '2024'].map((year) => (
    ...
    >
      ...
      <div className="grid grid-cols-6 border-t border-stroke py-4.5 px-4 dark:border-strokedark sm:grid-cols-8 md:px-6 2xl:px-7.5">
        ...
        <div className="col-span-1 flex items-center">
          <p className="font-medium">election_year</p>
        </div>
        <div className="col-span-1 flex items-center">
  ...
  ? regions.map((region) => (
      <div
        ...
        key={region?.region_id || 'default-key'}
      >
        <div className="col-span-1 flex items-center">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            <p className="text-sm text-black dark:text-white">
              {region?.region_id ? truncateText(region.region_id, 50) : '-'}
            </p>
          </div>
        </div>
        ...
        <div className="col-span-1 flex items-center">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            <p className="text-sm text-black dark:text-white">
              {region?.region_name ? truncateText(region.region_name, 50) : '-'}
            </p>
          </div>
        </div>
        ...
        <div className="col-span-1 flex items-center">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            <p className="text-sm text-black dark:text-white">
              {region?.initials ? truncateText(region.initials, 50) : '-'}
            </p>
          </div>
        </div>
        ...
        <div className="col-span-1 flex items-center">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            <p className="text-sm text-black dark:text-white">
              {region?.capital ? truncateText(region.capital, 50) : '-'}
            </p>
          </div>
        </div>
        ...
        <div className="col-span-1 flex items-center">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            <p className="text-sm text-black dark:text-white">
              {region?.election_year ? truncateText(region.election_year, 50) : '-'}
            </p>
          </div>
        </div>
                ...
                <p className="text-sm text-black dark:text-white">
                  <div className="flex items-center space-x-3.5">
                    ...
                    <button
                      ...
                      onClick={() => openDeleteModal(region.region_id)} // Pass the ID of the item to be deleted
                    >
                      <svg
                        className="fill-current"
                        width="18"
